---
title: 3. Basic Git Workflow
description: The four Git areas and the everyday commands that move your work through them.
---

import CommitFlow from '../../components/CommitFlow.svelte';
...
Press the button below to watch a change travel from your working directory all the way to the remote:

<CommitFlow client:visible />

Here's the same workflow, command by command.
...
#### Reviewing changes before you stage (git diff)

It's worth looking at exactly what changed before you stage it. `git diff` shows your **unstaged** changes (working directory vs. staging area), while `git diff --staged` shows what you've **already staged**, which is precisely what the next commit will contain:
...
For finer control, `git add -p` walks you through each change "hunk" and asks whether to stage it. This is how you split a messy working directory into small, logical commits:
...
We dedicate all of [Chapter 5](/git-primer/remotes/) to working with remotes.
